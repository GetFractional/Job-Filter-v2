...
function scoreWorkplaceType(jobPayload, userProfile) {
  ...
  const locationText = String(jobPayload.location || jobPayload.location_text || '').trim();
  ...
  if (!workplaceType) {
    return {
      ...
      actual_value: locationText || 'Not specified',
      score: 25,
  ...
  return {
    criteria: 'Work Location',
    criteria_description: 'Whether the job is remote, hybrid, or on-site based on your preferences',
    actual_value: [formatWorkplaceType(workplaceType), locationText].filter(Boolean).join(' · '),
    score: Math.round(score),
    rationale
  };
}
...
function scoreBenefits(jobPayload, userProfile) {
  ...
  const individualBenefits = {
    ...
    dental: {
      patterns: [/\bdental\b/i, /dental\s*(insurance|coverage|benefits|plan)/i],
      weight: 8,
      label: 'Dental'
    },
    vision: {
      patterns: [/\bvision\b/i, /vision\s*(insurance|coverage|benefits|plan)/i, /eye\s+care/i],
      weight: 6,
      label: 'Vision'
    },
    '401k': {
  ...
  if (allMatchedBenefits.length > 0) {
    actualValue = allMatchedBenefits.join(', ');
  } else if (matchedPreferredBenefits.length > 0) {
  ...
  return {
    ...
    benefit_badges: allMatchedBenefits.map(label => ({ label })),
    benefits_count: benefitsCount, // "X/Y" format for UI display
    missing_data: allMatchedBenefits.length === 0
  };
}
...
function scoreRoleType(jobPayload, userProfile) {
  ...
  if (!jobTitle || /assessing your job match|use ai to assess how you fit|show match details/.test(jobTitle)) {
    return {
      ...
      score: 0,
      rationale: 'Job title unavailable for role-type assessment',
      missing_data: true
    };
  }

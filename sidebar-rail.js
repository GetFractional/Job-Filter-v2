...
function buildSecondaryExtractionText(jobData) {
  const segments = [
    jobData?.jobTitle || '',
    jobData?.employmentType || '',
    jobData?.workplaceType || '',
    jobData?.location || '',
    ...(jobData?.featuredBenefits || []),
...
async function updateSkillCriteriaAsync(sidebar, scoreResult, jobData, userProfile) {
  const extractionText = buildSecondaryExtractionText(jobData);
  const analysisText = extractionText;
  ...
  try {
    if (analysisText.trim() && window.SkillExtractionService?.analyzeJobSkills) {
      const userSkills = userProfile?.background?.core_skills || [];
      const userTools = userProfile?.background?.tools || [];
      const analysis = await window.SkillExtractionService.analyzeJobSkills(analysisText, {
        jobUrl,
        userSkills,
        userTools,
        skipCache: true
      ...
      if (analysis && !analysis.error) {
        ...
        if (skillsItem) {
          ...
          const requiredSkills = dedupeLabels((analysis.extraction?.required || []).map(normalizeDesiredLabel).filter(Boolean));
          const desiredSkills = dedupeLabels((analysis.extraction?.desired || []).map(normalizeDesiredLabel).filter(Boolean));
          ...
          const desiredCount = desiredSkills.length;
          ...
          skillsItem.required_skills = requiredSkills;
          skillsItem.desired_skills = desiredSkills;
  ...
  await extractAndDisplayTools(sidebar, extractionText, userToJob, userProfile);
  if (shouldRerender || userToJob.some(item => item.criteria === 'Required Tools')) {
    updateScoreBreakdown(sidebar, scoreResult, userProfile);
  }
}
...
async function extractAndDisplayTools(sidebar, descriptionText, userToJobBreakdown, userProfile) {
  try {
    ...
    const hasSpecificSegmentToolContext = (value) => (
      /\b(twilio\s+segment|segment\s+cdp|segment\.com|customer\s+data\s+platform|cdp)\b/i.test(String(value || ''))
    );

    const isValidToolLabel = (value, evidence = '') => {
      const label = sanitizeToolLabel(value);
      if (!label) return false;
      const lower = label.toLowerCase();
      if (normalizeToolKey(label) === 'segment' && !hasSpecificSegmentToolContext(`${label} ${evidence}`)) return false;
    ...
    const termExistsInText = (term, text) => {
      if (!term || !text) return false;
      const escaped = escapeRegExp(term);
      const hasWordEdges = /^[a-z0-9 ]+$/i.test(term);
      const pattern = hasWordEdges
        ? new RegExp(`(^|[^a-z0-9])${escaped.replace(/\s+/g, '\\s+')}([^a-z0-9]|$)`, 'i')
        : new RegExp(escaped, 'i');
      return pattern.test(text);
    };

    const extractSupplementalToolsFromText = async (text) => {
      if (!text || !window.SkillClassifier?.loadToolsDictionary) return [];
      let toolsDictionary = [];
      try {
        toolsDictionary = await window.SkillClassifier.loadToolsDictionary();
      } catch (error) {
        console.warn('[Job Filter Sidebar] Supplemental tool dictionary scan failed:', error);
        return [];
      }
      if (!Array.isArray(toolsDictionary) || toolsDictionary.length === 0) return [];

      const matches = [];
      const ambiguousGenericToolTerms = new Set([
        'segment', 'make', 'mode', 'square', 'heap', 'stitch', 'sketch',
        'monday', 'teams', 'chorus', 'outreach', 'notion', 'airflow',
        'amplitude', 'census', 'copper', 'slack'
      ]);
      const ignoredShortAliases = new Set(['ga', 'sf', 'bq', 'xd']);
      for (const tool of toolsDictionary) {
        ...
        const terms = Array.from(new Set([
          ...
          .map((term) => sanitizeToolLabel(term))
          .filter((term) => term && term.length >= 2)));

        for (const term of terms) {
          const normalizedTerm = term.toLowerCase();
          if (ignoredShortAliases.has(normalizedTerm)) continue;
          if (ambiguousGenericToolTerms.has(normalizedTerm) && normalizedTerm !== 'segment') {
            continue;
          }
          if (normalizedTerm === 'segment' && !hasSpecificSegmentToolContext(`${term} ${text}`)) {
            continue;
          }
          if (!termExistsInText(term, text)) continue;
          if (normalizeToolKey(canonical || term) === 'segment' && !hasSpecificSegmentToolContext(`${term} ${text}`)) {
            continue;
          }
          matches.push({
            raw: term,
            name: term,
            canonical,
            evidence: `Supplemental tool dictionary text scan: ${term}`
          });
          break;
        }
      }
    ...
    const supplementalTools = await extractSupplementalToolsFromText(descriptionText);

    const normalizeToolItems = (items) => {
      const mapped = items
        .map((tool) => {
          const raw = typeof tool === 'string' ? tool : (tool.raw || tool.name || tool.canonical || '');
          const label = sanitizeToolLabel(raw);
          const evidence = typeof tool === 'string'
            ? tool
            : [tool.name, tool.raw, tool.canonical, tool.evidence, tool.context, tool.source].filter(Boolean).join(' ');
          if (!isValidToolLabel(label, evidence)) return null;
          const canonical = normalizeToolKey((typeof tool === 'string' ? '' : tool.canonical) || label);
    ...
    const requiredTools = normalizeToolItems([...(extraction.requiredTools || []), ...supplementalTools]);
    ...
    if (!toolsItem) {
      ...
      toolsItem = {
        ...
        actual_value: allTools.length > 0 && userToolList.length > 0
          ? `${matchedTools.length}/${allTools.length} tools (${Math.round((matchedTools.length / allTools.length) * 100)}%)`
          : (allTools.length > 0 ? `${allTools.length} tools found` : '0 tools found'),
        score: 0, // Placeholder until scoring engine returns a real result
        max_score: 50,
      ...
      if (!toolsItem.actual_value || toolsItem.actual_value.includes('tools found')) {
        toolsItem.actual_value = allTools.length > 0 && userToolList.length > 0
          ? `${matchedTools.length}/${allTools.length} tools (${Math.round((matchedTools.length / allTools.length) * 100)}%)`
          : (allTools.length > 0 ? `${allTools.length} tools found` : '0 tools found');
      }
    }
...
function renderBreakdownItems(breakdown, type, userProfile = null) {
  ...
  return filteredBreakdown.map(item => {
    const hasMissingData = item.missing_data === true;
    const score = item.score || 0;
    const criteriaMaxScore = item.max_score || 50;
    // Calculate percentage based on the criteria's max score
    const percentage = hasMissingData ? 0 : Math.round((score / criteriaMaxScore) * 100);
    ...
    if (item.criteria === 'Skills Overlap' && (item.matched_skills || item.unmatched_skills || totalUserSkills)) {
      ...
      const requiredSkills = dedupe(item.required_skills || []);
      const desiredSkills = dedupe(item.desired_skills || []);
      const matchedSkills = dedupe(item.matched_skills || []);
      const unmatchedSkills = dedupe(item.unmatched_skills || []);
      const matchCount = matchedSkills.length;
      const matchedSet = new Set(matchedSkills.map(normalizeKey));
      const displayRequiredSkills = requiredSkills.filter(s => matchedSet.has(normalizeKey(s)));
      const displayDesiredSkills = desiredSkills.filter(s => matchedSet.has(normalizeKey(s)));
      const displayedSkillKeys = new Set([...displayRequiredSkills, ...displayDesiredSkills].map(normalizeKey));
      const displayMatchedOnlySkills = matchedSkills.filter(s => !displayedSkillKeys.has(normalizeKey(s)));

      const extractedSkills = dedupe([...requiredSkills, ...desiredSkills]);
      const actualTotal = extractedSkills.length || (matchedSkills.length + unmatchedSkills.length);
      ...
      // Show only matched skills inline. Missing values stay in the hover tooltip.
          if (displayRequiredSkills.length > 0) {
            extraHtml += `
              <div class="jh-skill-tags jh-responsive-tags">
                ${displayRequiredSkills.map(s =>
          ...
          if (displayDesiredSkills.length > 0) {
            extraHtml += `
              <div class="jh-skill-tags jh-responsive-tags">
                ${displayDesiredSkills.map(s =>
                  `<span class="jh-skill-tag jh-matched">${escapeHtml(formatBadgeLabel(s))}</span>`
                ).join('')}
              </div>
            `;
          ...
          if (displayMatchedOnlySkills.length > 0) {
            extraHtml += `
              <div class="jh-skill-tags jh-responsive-tags">
                ${displayMatchedOnlySkills.map(s =>
                  `<span class="jh-skill-tag jh-matched">${escapeHtml(formatBadgeLabel(s))}</span>`
                ).join('')}
              </div>
            `;
      ...
      // Keep a compact missing summary for quick scanning.
      ...
      badgeCount = displayRequiredSkills.length + displayDesiredSkills.length + displayMatchedOnlySkills.length + (unmatchedSkills.length > 0 ? 1 : 0);
    }
    ...
    if (item.criteria === 'Benefits Package' || item.criteria === 'Benefits') {
      ...
      const detectedBenefits = Array.from(new Set(matchedBenefits));
      const displayMatched = matchedPreferred.length > 0 ? matchedPreferred : detectedBenefits;
      const matchCount = matchedPreferred.length;
      const displayBenefits = totalCount > 0 ? matchedPreferred : displayMatched;

      extraHtml += totalCount > 0
        ? `<div class="jh-benefits-summary">${matchCount}/${totalCount} benefits matched</div>`
        : `<div class="jh-benefits-summary">${detectedBenefits.length} benefits found</div>`;

      // Show only present/matched benefits inline. Missing preferences stay in the hover tooltip.
      ...
      badgeCount = displayBenefits.length + (missingPreferred.length > 0 ? 1 : 0);
    }
    ...
    if (item.criteria === 'Required Tools') {
      ...
      const normalizeToolDisplayKey = (value) => String(value || '')
        .toLowerCase()
        .replace(/^[•\-\*\u2022\u25E6\u25AA\u25CF·]+\s*/g, '')
        .replace(/\s+/g, '_')
        .replace(/[^a-z0-9_]/g, '')
        .trim();
      const matchedToolSet = new Set(matchedTools.map(normalizeToolDisplayKey));
      const displayRequiredTools = requiredTools.filter(t => matchedToolSet.has(normalizeToolDisplayKey(t)));
      const displayDesiredTools = desiredTools.filter(t => matchedToolSet.has(normalizeToolDisplayKey(t)));
      const displayedToolKeys = new Set([...displayRequiredTools, ...displayDesiredTools].map(normalizeToolDisplayKey));
      const displayMatchedOnlyTools = matchedTools.filter(t => !displayedToolKeys.has(normalizeToolDisplayKey(t)));
      ...
      if (allTools.length > 0) {
        ...
        extraHtml += `<div class="jh-tools-summary">0 tools found</div>`;
      }

      // Show only matched tools inline. Missing values stay in the hover tooltip.
      if (displayRequiredTools.length > 0) {
        extraHtml += `
          <div class="jh-tools-section">
            <div class="jh-tools-tags jh-responsive-tags">
              ${displayRequiredTools.map(t =>
      ...
      if (displayDesiredTools.length > 0) {
        extraHtml += `
          <div class="jh-tools-section jh-desired-tools">
            <div class="jh-tools-tags jh-responsive-tags">
              ${displayDesiredTools.map(t =>
      ...
      if (displayMatchedOnlyTools.length > 0) {
        extraHtml += `
          <div class="jh-tools-section">
            <div class="jh-tools-tags jh-responsive-tags">
              ${displayMatchedOnlyTools.map(t =>
                `<span class="jh-tool-tag jh-required">${escapeHtml(formatBadgeLabel(t))}</span>`
              ).join('')}
            </div>
          </div>
      ...
      badgeCount = displayRequiredTools.length + displayDesiredTools.length + displayMatchedOnlyTools.length + (missingTools.length > 0 ? 1 : 0);
    }
    ...
    const scoreDisplay = hasMissingData ? '--' : `${percentage}%`;
...
function setupJobsModeHandlers(sidebar) {
  ...
  if (huntBtn) {
    huntBtn.addEventListener('click', async () => {
      ...
      huntBtn.textContent = 'Saving...';
      huntBtn.classList.add('jh-loading');
      updateJobCaptureStatus(sidebar, {
        state: 'processing'
      });

      try {
        const response = await window.sendJobToAirtable(sidebarState.currentJobData, sidebarState.currentScore);
        ...
        if (response?.success) {
          ...
          updateJobCaptureStatus(sidebar, {
            ...
            companyRecordId: response.companyRecordId || null,
            contactRecordId: response.contactRecordId || null,
            baseId: response.baseId || null,
            pipeline: response.pipeline || null
          });
          if (response?.recordId) {
            void refreshGeneratedAssets(sidebar, response.recordId);
          }
          setTimeout(() => {
            huntBtn.textContent = 'Capture Job';
        ...
        huntBtn.textContent = 'Retry Capture';
        huntBtn.classList.remove('jh-loading');
        huntBtn.disabled = false;
        updateJobCaptureStatus(sidebar, {
          state: 'error',
  ...
  const copyRecordBtn = sidebar.querySelector('.jh-btn-copy-record');
  if (copyRecordBtn) {
    copyRecordBtn.addEventListener('click', async () => {
      const recordId = copyRecordBtn.getAttribute('data-record-id');
      if (!recordId) return;
      try {
        await navigator.clipboard.writeText(recordId);
        const previousLabel = copyRecordBtn.textContent;
        copyRecordBtn.textContent = 'Copied';
        setTimeout(() => {
          copyRecordBtn.textContent = previousLabel || 'Copy Job ID';
        }, 1600);
      } catch (err) {
        console.warn('[Job Filter Sidebar] Copy record ID failed:', err);
      }
    });
  }

  const refreshAssetsBtn = sidebar.querySelector('.jh-btn-refresh-assets');
  if (refreshAssetsBtn) {
    refreshAssetsBtn.addEventListener('click', async () => {
      const recordId = refreshAssetsBtn.getAttribute('data-record-id');
      if (!recordId) return;
      await refreshGeneratedAssets(sidebar, recordId);
    });
  }
}

function updateJobCaptureStatus(sidebar, status) {
  const statusEl = sidebar.querySelector('.jh-capture-status');
  const titleEl = sidebar.querySelector('.jh-capture-status-title');
  const bodyEl = sidebar.querySelector('.jh-capture-status-body');
  const detailsEl = sidebar.querySelector('.jh-capture-status-details');
  const actionsEl = sidebar.querySelector('.jh-capture-status-actions');
  const refreshAssetsBtn = sidebar.querySelector('.jh-btn-refresh-assets');
  const copyRecordBtn = sidebar.querySelector('.jh-btn-copy-record');
  if (!statusEl || !titleEl || !bodyEl || !detailsEl || !actionsEl) return;

  statusEl.classList.remove('jh-capture-idle', 'jh-capture-processing', 'jh-capture-success', 'jh-capture-error');
  detailsEl.innerHTML = '';

  if (status?.state === 'processing') {
    statusEl.classList.add('jh-capture-processing');
    titleEl.textContent = 'Capturing Job';
    bodyEl.textContent = 'Saving job data and preparing your asset workflow.';
    actionsEl.style.display = 'none';
    return;
  }

  if (status?.state === 'success') {
    statusEl.classList.add('jh-capture-success');
    titleEl.textContent = 'Capture Complete';
    const pipeline = status.pipeline || null;
    if (pipeline?.state === 'completed') {
      bodyEl.textContent = 'Asset workflow completed. Click "Refresh Assets" to load the generated assets.';
    } else if (pipeline?.state === 'running') {
      bodyEl.textContent = 'Asset workflow is running in Airtable. Refresh in a few seconds.';
    } else if (pipeline?.state === 'queued') {
      bodyEl.textContent = 'Asset workflow queued in Airtable. Refresh in 20-60 seconds.';
    } else if (pipeline?.state === 'not_configured') {
      bodyEl.textContent = 'Job captured. Asset workflow is not configured yet in Airtable.';
    } else if (pipeline?.state === 'disabled') {
      bodyEl.textContent = 'Job captured. Auto asset pipeline is disabled in Feature Flags.';
    } else if (pipeline?.state === 'failed') {
      bodyEl.textContent = `Job captured, but asset workflow failed: ${pipeline?.error || 'unknown error'}`;
    } else {
      bodyEl.textContent = 'Job captured. Refresh Assets to check workflow progress and output.';
    }
    const detailRows = [];
    if (status.recordId) detailRows.push(`<li><strong>Job ID:</strong> ${escapeHtml(status.recordId)}</li>`);
    if (pipeline?.state) detailRows.push(`<li><strong>Pipeline State:</strong> ${escapeHtml(String(pipeline.state))}</li>`);
    if (status.pipeline?.run_id) detailRows.push(`<li><strong>Pipeline Run:</strong> ${escapeHtml(status.pipeline.run_id)}</li>`);
    if (pipeline?.error) detailRows.push(`<li><strong>Pipeline Error:</strong> ${escapeHtml(String(pipeline.error))}</li>`);
    if (status.companyRecordId) detailRows.push(`<li><strong>Company ID:</strong> ${escapeHtml(status.companyRecordId)}</li>`);
    if (detailRows.length > 0) detailsEl.innerHTML = `<ul class="jh-capture-detail-list">${detailRows.join('')}</ul>`;
    actionsEl.style.display = 'flex';
    if (refreshAssetsBtn) {
      if (status.recordId) {
        refreshAssetsBtn.style.display = 'inline-flex';
        refreshAssetsBtn.setAttribute('data-record-id', status.recordId);
        refreshAssetsBtn.disabled = false;
        refreshAssetsBtn.textContent = 'Refresh Assets';
      } else {
        refreshAssetsBtn.style.display = 'none';
      }
    }
    if (copyRecordBtn) {
      if (status.recordId) {
        copyRecordBtn.style.display = 'inline-flex';
        copyRecordBtn.setAttribute('data-record-id', status.recordId);
      } else {
        copyRecordBtn.style.display = 'none';
      }
    }
    return;
  }
  ...
  if (status?.state === 'error') {
    statusEl.classList.add('jh-capture-error');
    titleEl.textContent = 'Capture Failed';
    bodyEl.textContent = status?.error || 'Could not save this job. Retry capture.';
    actionsEl.style.display = 'none';
    return;
  }

  statusEl.classList.add('jh-capture-idle');
  titleEl.textContent = 'After You Capture';
  bodyEl.textContent = 'Capture saves the job and starts your Airtable asset workflow.';
  detailsEl.innerHTML = `
    <ul class="jh-capture-detail-list">
      <li>Step 1: Job + event records are written to Airtable.</li>
      <li>Step 2: Airtable orchestration state is queued/running/completed.</li>
      <li>Step 3: Use "Refresh Assets" to load generated files into this panel.</li>
    </ul>
  `;
  actionsEl.style.display = 'none';
}

async function refreshGeneratedAssets(sidebar, jobRecordId) {
  const detailsEl = sidebar.querySelector('.jh-capture-status-details');
  const refreshAssetsBtn = sidebar.querySelector('.jh-btn-refresh-assets');
  if (!detailsEl || !jobRecordId) return;
  ...
  try {
    const response = await chrome.runtime.sendMessage({
      action: 'jobHunter.fetchGeneratedAssets',
      jobRecordId
    });

    if (!response?.success) {
      throw new Error(response?.error || 'Unable to load generated assets.');
    }
    ...
    if (!assets.length) {
      detailsEl.innerHTML = `
        <ul class="jh-capture-detail-list">
          <li><strong>Job ID:</strong> ${escapeHtml(jobRecordId)}</li>
          ${pipeline?.state ? `<li><strong>Pipeline State:</strong> ${escapeHtml(String(pipeline.state))}</li>` : ''}
          ${pipeline?.error ? `<li><strong>Pipeline Error:</strong> ${escapeHtml(String(pipeline.error))}</li>` : ''}
          <li>No generated assets yet. Wait 20-60 seconds and refresh again.</li>
        </ul>
      `;
      return;
    }
    ...
    const rows = assets.map((asset) => {
      const type = escapeHtml(asset.assetType || 'Asset');
      const url = asset.url ? escapeHtml(asset.url) : '';
      if (!url) return `<li>${type}</li>`;
      return `<li><a class="jh-asset-link" href="${url}" target="_blank" rel="noopener noreferrer">${type}</a></li>`;
    });
    ...
    detailsEl.innerHTML = `
      <ul class="jh-capture-detail-list">
        <li><strong>Job ID:</strong> ${escapeHtml(jobRecordId)}</li>
        ${pipeline?.state ? `<li><strong>Pipeline State:</strong> ${escapeHtml(String(pipeline.state))}</li>` : ''}
        <li><strong>Assets Ready:</strong> ${assets.length}</li>
        ${rows.join('')}
      </ul>
    `;
  } catch (error) {
    detailsEl.innerHTML = `
      <ul class="jh-capture-detail-list">
        <li><strong>Job ID:</strong> ${escapeHtml(jobRecordId)}</li>
        <li>Asset refresh failed: ${escapeHtml(error?.message || 'unknown error')}</li>
      </ul>
    `;
  } finally {
    if (refreshAssetsBtn) {
  ...
  return `
    <div class="jh-sidebar-container">
      ...
      <div class="jh-actions">
        <button class="jh-btn jh-btn-hunt jh-btn-primary">Capture Job</button>
      </div>
    </div>
  `;
}
...
function getSidebarStyles() {
  return `
    ...
    /* ========================================
       SKILL TAGS - Light badges for detected matches
       ======================================== */
    ...
    .jh-breakdown-item.jh-badge-card .jh-weight-inline {
      margin-top: 6px;
    }
    ...
    .jh-capture-status-body {
      color: #374151;
      margin-bottom: 8px;
    }

    .jh-capture-status-details {
      color: #4B5563;
    }

    .jh-capture-detail-list {
      margin: 0;
      padding-left: 18px;
    }

    .jh-capture-detail-list li {
      margin: 3px 0;
    }
    ...
    .jh-capture-status-actions {
      display: flex;
      gap: 8px;
      margin-top: 8px;
    }

    .jh-capture-status-actions .jh-btn {
      width: auto;
      max-width: none;
      flex: 1;
      padding: 8px 10px;
...
function updateBadgeOverflow(breakdownList) {
  ...
  tagSections.forEach((section) => {
    ...
    if (lastVisibleIndex < badges.length - 1) {
      ...
      moreBadge.className = `${section.classList.contains('jh-skill-tags') ? 'jh-skill-tag' : 'jh-benefit-tag'} jh-more jh-more-badge`;
...
function applySavedSidebarPosition(sidebar) {
  ...
  chrome.storage.local.get([SIDEBAR_POSITION_STORAGE_KEY], (result) => {
    ...
    const sidebarWidth = Math.min(rect.width || 400, Math.max(280, window.innerWidth - 16));
    const sidebarHeight = Math.min(rect.height || 500, Math.max(300, window.innerHeight - 16));
    const maxLeft = Math.max(0, window.innerWidth - sidebarWidth);
    const maxTop = Math.max(0, window.innerHeight - sidebarHeight);
    const savedLeft = Number(position.left);
    const savedTop = Number(position.top);
    const nextLeft = Number.isFinite(savedLeft) ? Math.min(Math.max(0, savedLeft), maxLeft) : maxLeft;
    const nextTop = Number.isFinite(savedTop) ? Math.min(Math.max(0, savedTop), maxTop) : 52;

    sidebar.style.left = `${nextLeft}px`;
    sidebar.style.top = `${nextTop}px`;
    sidebar.style.right = 'auto';
    sidebar.classList.remove('jh-hidden');
    sidebar.classList.add('jh-visible');
  });
}
...
function enableSidebarResize(sidebar) {
  ...
  if (chrome?.storage?.local) {
    chrome.storage.local.get([SIDEBAR_SIZE_STORAGE_KEY], (result) => {
      ...
      if (size) {
        if (size.width) {
          const width = Math.min(Math.max(MIN_WIDTH, Number(size.width) || MIN_WIDTH), Math.min(MAX_WIDTH, window.innerWidth - 16));
          sidebar.style.width = `${width}px`;
        }
        if (size.height) {
          const height = Math.min(Math.max(MIN_HEIGHT, Number(size.height) || MIN_HEIGHT), window.innerHeight - 16);
  ...
  if (request.action === 'jobCaptureComplete') {
    ...
    if (request.success) {
      ...
      updateJobCaptureStatus(sidebar, {
        ...
        companyRecordId: request.companyRecordId || null,
        contactRecordId: request.contactRecordId || null,
        baseId: request.baseId || null,
        pipeline: request.pipeline || null
      });
      if (request.recordId) {
        void refreshGeneratedAssets(sidebar, request.recordId);
      }
      ...
      updateJobCaptureStatus(sidebar, {
        state: 'error',
        error: request.error || 'Failed to save this job to Airtable.'
      });

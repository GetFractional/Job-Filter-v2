...
- no implementation handoff without build-readiness verdict and screen contract
...
| --- | --- | --- |
| Governance and sequencing | GPT-5.5 lead thread | packets, ownership decisions, QA verdicts, prompt docs, screen contracts |
| Repo-grounded implementation | gpt-5.3-codex coding thread | code or docs diff, verification, risks |
...
4. Produce a build-readiness verdict, source-of-truth map, screen contract, acceptance criteria, file scope, test plan, QA/audit plan, and rollback plan.
5. Implement only when the readiness verdict is `unblocked` and the packet-approved scope is clear.
6. Verify locally before claiming completion.
7. If ClickUp or GitHub is in scope, mutate only after the diff is understood and re-fetch to prove the change.
